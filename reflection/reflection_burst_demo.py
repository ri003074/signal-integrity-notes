...
def v_inc(t_arr: np.ndarray) -> np.ndarray:
    ...
    sin_part   = (AMPLITUDE / 2 + AMPLITUDE / 2 * np.sin(
        2 * np.pi * FREQ_HZ * (t_arr - fix_end) + np.pi / 2)) * sin_gate
...
def draw_circuit(ax: plt.Axes) -> None:
    ...
    ax.add_patch(mpatches.FancyBboxPatch(
        (0.2, LINE_MID - 0.7), 1.6, 1.4,
        boxstyle="round,pad=0.15", facecolor="lightcyan",
    ...
    ax.plot([TL1_X0, TL1_X0], [LINE_Y_BOT + 0.4, LINE_Y_TOP - 0.4],
            "-", color="black", linewidth=1.5)
    ax.plot([TL1_X1, TL1_X1], [LINE_Y_BOT + 0.4, LINE_Y_TOP - 0.4],
            "-", color="black", linewidth=1.8)
    ax.text((TL1_X0 + TL1_X1) / 2, LINE_MID,
            f"TL1\n$Z_1={Z1:.0f}\\,\\Omega$\n$T_{{D1}}={TD1_NS}\\,ns$",
    ...
    ax.plot([TL2_X0, TL2_X0], [LINE_Y_BOT + 0.4, LINE_Y_TOP - 0.4],
            "-", color="black", linewidth=1.8)
    ax.plot([TL2_X1, TL2_X1], [LINE_Y_BOT + 0.4, LINE_Y_TOP - 0.4],
            ":", color="darkorchid", linewidth=1.5)
    ax.text((TL2_X0 + TL2_X1) / 2, LINE_MID,
            f"TL2\n$Z_2={Z2:.0f}\\,\\Omega$\n$T_{{D2}}={TD2_NS}\\,ns$",
            ha="center", va="center", fontsize=8.5,
    ...
    ax.annotate(f"$\\Gamma_J={GAMMA_J:+.3f}$\n接続点",
                xy=(TL1_X1, LINE_MID), xytext=(TL1_X1 - 0.5, 4.9),
    ...
    ax.annotate("観測点 $V_{obs}$\n(受信端)",
                xy=(10.2, LINE_MID), xytext=(8.8, 4.6),
                fontsize=9.5, color="darkorchid",
                arrowprops=dict(arrowstyle="-|>", color="darkorchid", lw=1.5),
                bbox=dict(boxstyle="round,pad=0.25", facecolor="#f3e5f5",
                          edgecolor="darkorchid", alpha=0.95))
    ...
    # ── RL ボックス
    ax.add_patch(mpatches.FancyBboxPatch(
        (10.8, LINE_MID - 0.45), 1.4, 0.9,
        boxstyle="round,pad=0.1", facecolor="lightyellow",
        edgecolor="darkgoldenrod", linewidth=2))
    ax.text(11.5, LINE_MID, f"$R_L={ZL:.0f}\\,\\Omega$",
            ha="center", va="center", fontsize=9)
    ...
    # ── 入射波矢印（TL1 と TL2 内）
    ax.annotate("", xy=(9.4, LINE_Y_TOP), xytext=(5.0, LINE_Y_TOP),
                arrowprops=dict(arrowstyle="-|>", color="royalblue", lw=2.0))
    ax.text(7.2, LINE_Y_TOP + 0.25, "入射波 $V_{inc}(t)$",
            color="royalblue", ha="center", fontsize=9)

    # ── 反射波矢印（接続点反射 + 負荷反射）
    ax.annotate("", xy=(5.0, LINE_Y_BOT), xytext=(9.4, LINE_Y_BOT),
                arrowprops=dict(arrowstyle="-|>", color="crimson", lw=2.0))
    ax.text(7.2, LINE_Y_BOT - 0.3,
            f"反射波（接続点：$\\Gamma_J={GAMMA_J:+.3f}$、負荷：$\\Gamma_L={GAMMA_L:+.3f}$）",
            color="crimson", ha="center", fontsize=8.5)
...
ax.text(FIX_END_NS + 0.1, -0.1, f"Sin 開始\n$t={FIX_END_NS:.0f}$ ns",
        color="royalblue", fontsize=8.5)

# Sin→後半フラット切替ライン
ax.axvline(SIN_END_NS, color="royalblue", linestyle=":", linewidth=1.5, alpha=0.8)
ax.text(SIN_END_NS + 0.1, -0.1, f"Sin 終了\n$t={SIN_END_NS:.0f}$ ns",
        color="royalblue", fontsize=8.5)

# 区間ラベル
ax.text(TD1_NS, 0.72, "反射未到達\n(クリーンな波形)",
        ha="center", va="center", fontsize=10, color="#1b5e20",
        bbox=dict(boxstyle="round,pad=0.3", facecolor=ZONE_NO_REF_COLOR,
                  edgecolor="#388e3c", alpha=0.95))
ax.text((FIRST_REF_NS + PLOT_END_NS) / 2, 0.72, "反射が届く区間\n(この後で波形が変化)",
        ha="center", va="center", fontsize=10, color="#e65100",
        bbox=dict(boxstyle="round,pad=0.3", facecolor=ZONE_WITH_REF_COLOR,
                  edgecolor="#f57c00", alpha=0.95))

# バースト終了マーカ
ax.axvline(BURST_END_NS, color="gray", linestyle=":", linewidth=1.2, alpha=0.7)
ax.text(BURST_END_NS + 0.1, -0.1, f"バースト終了\n$t={BURST_END_NS:.0f}$ ns",
        color="gray", fontsize=8.5)

# ── 反射波 ───────────────────────────────────────────
ax = ax_ref
shade_zones(ax)
ax.plot(t_ns, vr, color="crimson", linewidth=2.2, zorder=4,
...
ax.annotate(
    ...
    xy=(LOAD_REF_NS, 0.0), xytext=(LOAD_REF_NS + 1.0, 0.15),
...
ax_ckt.text(FIRST_REF_NS + 0.07, -0.1, f"←  接続点反射到達\n   $t=2T_{{D1}}={FIRST_REF_NS:.0f}$ ns",
            color="dimgray", fontsize=8.5)
...
# 入射波（薄く参考）
ax.plot(t_ns, vi, color="royalblue", linewidth=1.2, alpha=0.3,
        linestyle="--", zorder=3, label="入射波のみ（参考, 遅延なし表示）")

# 受信端波形
ax.plot(t_ns, vrx, color="darkorchid", linewidth=2.5, zorder=4,
...
ax.fill_between(t_ns, 0, vrx, where=rx_mask,
                color="#ce93d8", alpha=0.25, zorder=2)
...
# 信号未到達ラベル
ax.text(OBS_ARRIVE_NS * 0.5, 0.25, "信号未到達\n(ゼロ)",
        ha="center", va="center", fontsize=9, color="#880e4f",
        bbox=dict(boxstyle="round,pad=0.25", facecolor="#fce4ec",
...
ax.annotate(
    ...
    xy=(peak_t, peak_v), xytext=(peak_t + 1.5, peak_v - 0.1),
    fontsize=9.5, color="darkorchid",
    arrowprops=dict(arrowstyle="-|>", color="darkorchid", lw=1.5),
...
ax.text(OBS_ARRIVE_NS + 0.1, -0.12,
        f"$t=T_{{D1}}+T_{{D2}}={OBS_ARRIVE_NS}$ ns 以降:\n観測点に信号が届き始める",
        color="#4527a0", fontsize=8.5,
        bbox=dict(boxstyle="round,pad=0.25", facecolor="#e8eaf6",
                  edgecolor="#5e35b1", alpha=0.9))
